...
def api_create_reserva():
    ...
    cursor.execute("SELECT tokens FROM users WHERE id = ?", (alumno_id,))
...
def api_get_reserva_activa():
    ...
    return jsonify({"reserva": serialize_reserva(reserva)}), 200
...
def confirmar_reserva(reserva_id, campo):
    conn = get_db()
    cursor = conn.cursor()
    ...
    if (
        ...
        cursor.execute("SELECT profesor_id FROM reservas WHERE id = ?", (reserva_id,))
        reserva = cursor.fetchone()
    ...
    return serialize_reserva(reserva_actualizada), None
...
def api_confirmar_alumno(reserva_id):
    ...
    reserva, error = confirmar_reserva(reserva_id, "confirmacion_alumno")
...
def api_confirmar_profesor(reserva_id):
    ...
    reserva, error = confirmar_reserva(reserva_id, "confirmacion_profesor")
